...
const fixValues = (obj = {}) => {
    ...
    Object.keys(obj).forEach((key) => {
        if (!processed.includes(key) && !exclusions.includes(key) && !range.includes(key)) {
            ...
            if (newValue.match(/M/g)) {
                newValue = Math.floor(Number.parseFloat(newValue.replace(/M/g, '')) * 1.0e+6)
            } else if (newValue.match(/B/g)) {
                newValue = Math.floor(Number.parseFloat(newValue.replace(/B/g, '')) * 1.0e+9)
            } else {
    ...
    range.forEach((k) => {
        ...
        if (k === 'volatility') {
            obj[k] = {
                week: Number.parseFloat(rangeArr[0]),
                month: Number.parseFloat(rangeArr[1])
            }
        } else {
            obj[k] = {
...
const getStock = async (ticker = '') => {
    try {
        ...
        console.log(insidersTable.length)
